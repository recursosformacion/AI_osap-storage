from __future__ import annotations

from collections.abc import Callable
from typing import Any, cast

from domain.ports.repositories import StorageProviderRepository
from fastapi import Request
from infrastructure.config import Settings
from infrastructure.container import Container
from infrastructure.db.connection import Database


def get_container(request: Request) -> Container:
    return cast(Container, request.app.state.container)


def get_settings(request: Request) -> Settings:
    return cast(Container, request.app.state.container).settings


def get_db(request: Request) -> Database:
    return cast(Container, request.app.state.container).db


def get_provider_repo(request: Request) -> StorageProviderRepository:
    return cast(Container, request.app.state.container).provider_repo


def get_file_repo(request: Request):
    return cast(Container, request.app.state.container).file_repo


def get_archive_entry_repo(request: Request):
    return cast(Container, request.app.state.container).archive_entry_repo


def _use_case(attr: str) -> Callable[[Request], Any]:
    def resolver(request: Request) -> Any:
        return cast(Container, request.app.state.container).__getattribute__(attr)

    return resolver


RegisterFileDep = _use_case("register_file")
GetFileDep = _use_case("get_file")
ListFilesDep = _use_case("list_files")
StartDownloadDep = _use_case("start_download")
GetDownloadJobDep = _use_case("get_download_job")
GetDownloadUrlDep = _use_case("get_download_url")
StreamFileDep = _use_case("stream_file")
CreateProviderDep = _use_case("create_provider")
GetProviderDep = _use_case("get_provider")
ListProvidersDep = _use_case("list_providers")
ResolveFileDep = _use_case("resolve_file")
SearchEntriesDep = _use_case("search_entries")
VerifyFileDep = _use_case("verify_file")
DeleteFileDep = _use_case("delete_file")
ListArchivesDep = _use_case("list_archives")
GetArchiveDep = _use_case("get_archive")
CountMissingEntriesDep = _use_case("count_missing_entries")
GetStatisticsDep = _use_case("get_statistics")
SearchWorksDep = _use_case("search_works")
SearchWorksFullDep = _use_case("search_works_full")
GetWorkDep = _use_case("get_work")
ListComposersDep = _use_case("list_composers")
CatalogueQueriesDep = _use_case("catalogue_queries")
TableCrudDep = _use_case("table_crud")
GetComposerDetailDep = _use_case("get_composer_detail")
GetComposerWorksDep = _use_case("get_composer_works")
MergeComposersDep = _use_case("merge_composers")
CreateComposerDep = _use_case("create_composer")
ReviewComposerDep = _use_case("review_composer")
AddAliasDep = _use_case("add_alias")
ListAliasesDep = _use_case("list_aliases")
MoveAliasDep = _use_case("move_alias")
PromoteAliasDep = _use_case("promote_alias")
SetAttributionDep = _use_case("set_attribution")
ComposerReviewStatsDep = _use_case("composer_review_stats")
ClassifyComposersDep = _use_case("classify_composers")
UpdateComposerDep = _use_case("update_composer")
GetComposerBiographyDep = _use_case("get_composer_biography")
UpdateComposerBiographyDep = _use_case("update_composer_biography")
DeleteComposerIdentifierDep = _use_case("delete_composer_identifier")
ListWorksAdminDep = _use_case("list_works_admin")
GetWorkAdminDep = _use_case("get_work_admin")
UpdateWorkAdminDep = _use_case("update_work_admin")
RecordVoteDep = _use_case("record_vote")
GetWorkStatisticsDep = _use_case("get_work_statistics")
GetComposerStatisticsDep = _use_case("get_composer_statistics")
RefreshVotingStatisticsDep = _use_case("refresh_voting_statistics")
