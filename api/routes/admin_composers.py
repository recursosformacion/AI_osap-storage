...
async def get_composer(
    ...
    return ComposerAdminDetail.model_validate(await uc.execute(person_id))
...
async def delete_composer_identifier(
    ...
    await uc.execute(person_id, identifier_id)
...
async def composer_works(
    ...
    works, total = await uc.execute(person_id, limit=limit, offset=offset)
...
async def review_composer(
    ...
    detail = await uc.execute(person_id, payload.review_status)
    return ComposerAdminDetail.model_validate(detail)
...
async def add_alias(
    ...
    alias = await uc.execute(person_id, payload.alias)
...
async def move_alias(
    ...
    alias = await uc.execute(alias_id, payload.from_person_id, payload.target_person_id)
...
@router.post(
    "/{person_id}/aliases/{alias_id}/promote",
...
async def promote_alias(
    ...
    composer = await uc.execute(alias_id, person_id)
    return PromoteAliasResultRead(person_id=composer.id, name=composer.name)
...
@router.post(
    ...
    "(= nombre) y se les borra person_id; los compositores se retiran.",
)
async def set_attribution(
    payload: SetAttributionRequest,
    uc: SetAttribution = Depends(SetAttributionDep),
) -> SetAttributionResultRead:
    affected = await uc.execute(payload.person_ids, payload.attribution_type)
