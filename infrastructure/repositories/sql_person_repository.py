...
class SqlPersonRepository(ComposerRepository):
    ...
    async def get_by_id(self, person_id: str) -> Composer | None:
        async with self._db.connection() as conn, conn.cursor() as cur:
            await cur.execute(
                f"SELECT {_COMPOSER_COLS} FROM persons WHERE persons_id = %s", (person_id,)
            )
        ...
        try:
            async with self._db.connection() as conn, conn.cursor() as cur:
                await cur.execute(
                    ...
                    (person_id, alias, normalized_alias),
                )
                await cur.execute(
    ...
    async def set_musicbrainz_id(self, person_id: str, musicbrainz_id: str | None) -> None:
        ...
        async with self._db.connection() as conn, conn.cursor() as cur:
            ...
            if musicbrainz_id:
                await cur.execute(
                    "SELECT persons_name FROM persons WHERE persons_id = %s", (person_id,)
                )
                name_row = await cur.fetchone()
                name = str((name_row or {}).get("persons_name") or "")
                await cur.execute(
        ...
        async with self._db.connection() as conn, conn.cursor() as cur:
            if suspicious:
                await cur.execute(
                    ...
                    "WHERE persons_id = %s",
                    (reason or "suspicious", person_id),
                )
            else:
                await cur.execute(
    ...
    async def record_resolution(self, resolution: ComposerResolution) -> ComposerResolution:
        ...
        person_id = resolution.candidate_person_id or resolution.old_person_id
        if person_id is None:
            return resolution
        anchor_value = f"work:{resolution.work_id}"
        payload = _json.dumps({
            "work_id": resolution.work_id,
            "old_person_id": resolution.old_person_id,
            "candidate_person_id": resolution.candidate_person_id,
        ...
        async with self._db.connection() as conn, conn.cursor() as cur:
            await cur.execute(
                ...
                (person_id, resolution.decision, resolution.reason, anchor_value,
    ...
    async def list_resolutions(self, work_id: int) -> list[ComposerResolution]:
        ...
        for r in rows:
            ...
            result.append(ComposerResolution(
                ...
                old_person_id=payload.get("old_person_id"),
    ...
    async def add_evidence(
        ...
        async with self._db.connection() as conn, conn.cursor() as cur:
            await cur.execute(
                ...
                "VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                (person_id, rule, decision, reason, anchor_type, anchor_value,
    ...
    async def rename_composer(self, person_id: str, new_name: str) -> None:
        ...
        async with self._db.connection() as conn, conn.cursor() as cur:
            ...
            await cur.execute(
                ...
                "VALUES (%s, %s, %s)",
                (person_id, new_name, normalize_composer_name(new_name)),
            )
    ...
    async def update_composer(
        ...
        if musicbrainz_id is not None:
            await self.set_musicbrainz_id(person_id, musicbrainz_id)
        if cluster_id is not None:
            await self.add_identifier(person_id, "cluster", cluster_id, source="maestro")
        if name is not None:
            from domain.services.composer_names import normalize_composer_name
    ...
    async def get_detail(self, person_id: str) -> ComposerDetail | None:
        async with self._db.connection() as conn, conn.cursor() as cur:
            await cur.execute(
            ...
            await cur.execute(
                "SELECT id, persons_id AS person_id, persons_evidence_rule AS rule, "
                "persons_evidence_decision AS decision, persons_evidence_reason AS reason, "
                "persons_evidence_anchor_type AS anchor_type, "
                "persons_evidence_anchor_value AS anchor_value, "
                "persons_evidence_channels AS channels, "
    ...
    async def list_works(
        ...
        async with self._db.connection() as conn, conn.cursor() as cur:
            ...
            return [
                ComposerWorkRef(
                    work_id=r["work_id"], title=r["title"], person_id=r["person_id"]
                )
                for r in await cur.fetchall()
            ]
    ...
    async def move_alias(self, alias_id: int, target_id: str, from_person_id: str) -> ComposerAlias:
        ...
        (si no existe). Las obras cuyo person_id=origen y composer=alias pasan al destino.
        """
        async with self._db.transaction() as conn, conn.cursor() as cur:
            await cur.execute(
                f"SELECT {_ALIAS_COLS} FROM persons_aliases "
                "WHERE id = %s AND person_id = %s",
                (alias_id, from_person_id),
            )
            row = await cur.fetchone()
            if row is None:
            ...
            return ComposerAlias(
                person_id=target_id, alias=alias.alias,
                normalized_alias=alias.normalized_alias, id=None,
            )

    async def promote_alias(self, alias_id: int, from_person_id: str) -> Composer:
    ...
    async def set_attribution(self, person_ids: list[str], attribution_type: str) -> int:
        ...
        Se borra person_id de sus obras y se guarda attribution_type + attribution_note
    ...
    async def _canonical_of(self, conn, person_id: str, first_row: dict) -> tuple[str, str]:
        canonical = await self._canonical_map([person_id])
    ...
    async def _canonical_map(
        ...
        if not person_ids:
            return {}
        unique: list[str] = list(dict.fromkeys(person_ids))
...
# Compatibilidad: nombre histórico de la clase.
