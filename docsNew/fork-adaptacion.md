...
- [x] `sql_composer_repository.py` **completo**: todo el repositorio (lectura y escritura) sobre `persons_*` + `works_person_roles` (rol 1): listado/conteos/detalle, alta, alias (añadir/listar/mover/promover), identificadores, evidencia, biografía, `merge`, `set_review_status`, `set_suspicious`, `set_musicbrainz_id`, `update_composer`, `set_attribution`, `resolve_*`, `backfill_creation_evidence`, `prune_zero_work_composers`, `find_by_identifier`, etc. `musicbrainz_id`/`cluster_id` pasan a `persons_identifiers`.
- [ ] Resto de repositorios: `sql_voting_repository.py`, `sql_catalogue_repository.py` (`catalog`), `sql_authority_*` (`persons_authority*`), `sql_job_repository`, `sql_statistics_repository`.
